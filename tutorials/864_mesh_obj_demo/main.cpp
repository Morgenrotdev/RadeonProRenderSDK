...
struct Context
{
	...
	std::string outFileName = "864.png";
};
...
void loadAndAttachShapes(rpr_context& context, rpr_scene& scene, rpr_material_system matsys, const ShapeSettings& settings, const std::vector<AtomData>& atoms)
{
	...
	for (size_t i = 0; i < shapes.size(); ++i)
	{
		...
		rpr_shape t_shape = nullptr;
		{
			...
			CHECK(rprShapeSetVisibility(t_shape, RPR_FALSE)); //Unvisible the obj sphere
			CHECK(rprShapeSetTransform(t_shape, RPR_TRUE, &m.m00));
...
void printHelp()
{
	...
	std::cout << "864_rpr_mesh_obj_demo.exe cfg.json [-h | --help]" << '\n';
	return;
}
...
int main()
{
	...
	//202310
	//const char* filepath = "../64_mesh_obj_demo/sphere.json";
	const char* filepath = "../864_mesh_obj_demo/sphere_original_camera.json";
	...
	const Configuration config = loadConfigFile(filepath);
